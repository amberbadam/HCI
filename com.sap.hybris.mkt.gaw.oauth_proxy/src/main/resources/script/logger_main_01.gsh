...
def Message processData(Message message) {
    Logger log = LoggerFactory.getLogger(this.getClass());
     ...
     try {
        processData("oauth_proxy_main_00", message);
    } catch (Exception ex) {
...
def Message processData01(Message message) {
    Logger log = LoggerFactory.getLogger(this.getClass());
   
     try {
        processData("oauth_proxy_main_01", message);
    } catch (Exception ex) {
       log.error("processData error",ex);
    }
}
...
def Message processData02(Message message) {
    Logger log = LoggerFactory.getLogger(this.getClass());
   
     try {
        processData("oauth_proxy_main_02", message);
    } catch (Exception ex) {
       log.error("processData error",ex);
    }
}

def Message processData03(Message message) {
    Logger log = LoggerFactory.getLogger(this.getClass());
   
     try {
        processData("oauth_proxy_main_03", message);
    } catch (Exception ex) {
       log.error("processData error",ex);
    }
}

def Message processData04(Message message) {
    Logger log = LoggerFactory.getLogger(this.getClass());
   
     try {
        processData("oauth_proxy_main_04", message);
    } catch (Exception ex) {
       log.error("processData error",ex);
    }
}
